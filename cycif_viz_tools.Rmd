...
files <- list.files(path = "N:lsp-data/cycif-production/164-CRC_Giannakis/P164_B462_CyCIF/csvs", full.names = TRUE)
...
summary(sctables$LSP20720()$CD56)

# Downsample to 10,000 cells for visualization
set.seed(123)  
sample_data <- sctables$LSP20720()[sample(nrow(sctables$LSP20720()), 10000), ]

ggplot(sample_data, aes(x = X_centroid, y = Y_centroid, color = CD56)) +
  geom_point(size = 0.5) +
  scale_color_gradient(low = "blue", high = "red") +
  theme_minimal() +
  labs(title = "Scatter Plot of CD56 Expression",
       x = "X Coordinate",
       y = "Y Coordinate",
       color = "CD56 Expression")
...
marker_data <- sctables$LSP20720()[, 2:29]
marker_data <- marker_data[, -c(1,5,9,14,19,24)] # removing the Hoechsts
...
visual_gate <- function(data, channel, mgate = NULL) {
  ...
  return(list(gate = gate_info$gate))
}
...
visual_gate(sctables$LSP20720(), "panCK")
...
toGate <- toGate[!grepl("Hoechst", toGate)]
...
gating_loop <- function(data, allChs, refGate, refGateValue = NULL) {
  ...
  for (i in seq_along(allChs)) {
    ...
    if (setGate == refGate) {
      ...
      gatevals[i] <- result_2D$gate2  # Save the gate value for the current marker
...
gates <- fread("C:/Users/kq18/HMS Dropbox/Kristin Qian/data/dMMR/CyCIF_gates.csv")
...
for (file_name in names(sctables)) {
  ...
  if (grepl("^LSP", file_name)) {
    data <- sctables[[file_name]]()
  
    # Create a file name for the CSV
    csv_file_name <- paste0(file_name, ".p.csv")
...
write.csv(sctables$LSP20720(), "LSP10720.p.csv", row.names = FALSE)
